...
pub async fn visita_campo_agenda(
    ...
    let carnet = query.get("carnet").cloned().unwrap_or_else(|| user.carnet().to_string());
    ...
    let res = match client.query("EXEC sp_vc_agenda_hoy_rust @P1, @P2, @P3", &[&carnet, &lat, &lon]).await {
        Ok(stream) => {
                    ...
                    let items: Vec<serde_json::Value> = rows.into_iter().map(|r| {
                        ...
                        for (i, col) in r.columns().iter().enumerate() {
                            ...
                            if let Ok(Some(i_val)) = r.try_get::<i32, _>(i) {
                                ...
                                map.insert(name, serde_json::json!(d_val.format("%Y-%m-%d %H:%M:%S").to_string()));
                            } else {
...
pub async fn visita_campo_clientes(
    ...
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Visita_ObtenerClientes_rust", &[]).await;
    Json(serde_json::json!({"success": true, "items": items})).into_response()
}
...
pub async fn visita_campo_checkin(
    ...
    let carnet = body.get("carnet").and_then(|v| v.as_str()).unwrap_or(user.carnet());
    ...
    let timestamp = body.get("timestamp").and_then(|v| v.as_str()).map(|s| chrono::NaiveDateTime::parse_from_str(s, "%Y-%m-%dT%H:%M:%S%.fZ").unwrap_or_else(|_| chrono::Utc::now().naive_utc()));
    let agenda_id = body.get("agenda_id").and_then(|v| v.as_i64()).map(|i| i as i32);
    let offline_id = body.get("offline_id").and_then(|v| v.as_str());

    match client.execute(
        "EXEC sp_vc_checkin_rust @P1, @P2, @P3, @P4, @P5, @P6, @P7, @P8",
        &[&carnet, &cliente_id, &lat, &lon, &accuracy, &timestamp, &agenda_id, &offline_id],
    ).await {
        Ok(_) => Json(serde_json::json!({"success": true, "message": "Check-in registrado via DB"})).into_response(),
        Err(e) => Json(serde_json::json!({"error": e.to_string()})).into_response(),
    }
}
...
pub async fn visita_campo_checkout(
    ...
    let visita_id = body.get("visita_id").or(body.get("id_visita")).and_then(|v| v.as_i64()).unwrap_or(0) as i32;
    let carnet = body.get("carnet").and_then(|v| v.as_str()).unwrap_or(user.carnet());
    let lat = body.get("lat").and_then(|v| v.as_f64());
    let lon = body.get("lon").and_then(|v| v.as_f64());
    let accuracy = body.get("accuracy").and_then(|v| v.as_f64());
    let timestamp = body.get("timestamp").and_then(|v| v.as_str()).map(|s| chrono::NaiveDateTime::parse_from_str(s, "%Y-%m-%dT%H:%M:%S%.fZ").unwrap_or_else(|_| chrono::Utc::now().naive_utc()));
    let observacion = body.get("observacion").or(body.get("notas")).and_then(|v| v.as_str());
    ...
    match client.execute(
        ...
        &[&visita_id, &carnet, &lat, &lon, &accuracy, &timestamp, &observacion, &foto_path, &firma_path],
    ).await {
        Ok(_) => Json(serde_json::json!({"success": true, "message": "Check-out registrado via DB"})).into_response(),
        Err(e) => Json(serde_json::json!({"error": e.to_string()})).into_response(),
    }
}
...
pub async fn visita_campo_resumen(
    ...
    let res = match client.query("EXEC sp_vc_resumen_dia_rust @P1, NULL", &[&carnet]).await {
        Ok(s) => match s.into_first_result().await {
            Ok(rows) => {
                if let Some(r) = rows.into_iter().next() {
                    let v_comp = r.try_get::<i32, _>("visitas_completadas").ok().flatten().unwrap_or(0);
                    let v_pend = r.try_get::<i32, _>("visitas_pendientes").ok().flatten().unwrap_or(0);
                    let efectividad = r.try_get::<f64, _>("efectividad").ok().flatten().unwrap_or(0.0);
            ...
            Err(_) => Json(serde_json::json!({"success": true, "visitas_completadas": 0})).into_response(),
        },
        Err(_) => Json(serde_json::json!({"success": true, "visitas_completadas": 0})).into_response(),
    };
    res
...
pub async fn visita_campo_tracking_batch(
    ...
    let puntos = body.get("puntos").unwrap_or(&serde_json::json!([])).to_string();

    match client.execute("EXEC sp_vc_tracking_batch_rust @P1, @P2", &[&carnet, &puntos]).await {
...
pub async fn visita_campo_stats_km(
    ...
    match client.query("EXEC sp_vc_calculo_km_dia_rust @P1, @P2", &[&carnet, &fecha]).await {
        Ok(s) => match s.into_first_result().await {
            Ok(rows) => {
                if let Some(r) = rows.into_iter().next() {
...
pub async fn visita_campo_tracking_raw(
    ...
    match client.query("EXEC sp_vc_tracking_por_dia_rust @P1, @P2", &[&carnet, &fecha]).await {
        Ok(s) => match s.into_first_result().await {
            Ok(rows) => {
...
pub async fn visita_campo_usuarios_tracking(
    ...
    match client.query("EXEC sp_vc_usuarios_con_tracking_rust", &[]).await {
        Ok(s) => match s.into_first_result().await {
            Ok(rows) => {
                let items: Vec<serde_json::Value> = rows.into_iter().map(|r| crate::handlers::equipo::row_to_json(&r)).collect();
                return Json(items).into_response();
...
pub async fn visita_admin_dashboard(
    ...
    // Zero Inline SQL: Usar sp_Visita_ObtenerStats
    let stats = crate::handlers::equipo::exec_sp_to_json(
        &mut client, 
    ...
    if let Some(r) = stats.first() {
        Json(serde_json::json!({
            "success": true,
            "metrics": {
                "total_hoy": r.get("visitas_hoy").and_then(|v| v.as_i64()).unwrap_or(0),
                "completadas_hoy": r.get("completadas_hoy").and_then(|v| v.as_i64()).unwrap_or(0),
                "clientes_activos": r.get("clientes_activos").and_then(|v| v.as_i64()).unwrap_or(0),
                "alertas_fuera_zona": r.get("alertas_fuera_zona").and_then(|v| v.as_i64()).unwrap_or(0),
            }
        })).into_response()
    } else {
...
pub async fn visita_admin_visitas(
    _user: crate::auth::AuthUser,
    State(state): State<ApiState>,
    _query: axum::extract::Query<HashMap<String, String>>,
) -> impl IntoResponse {
    let mut client = match state.pool.get().await {
        Ok(c) => c,
        Err(e) => return Json(serde_json::json!({"error": e.to_string()})).into_response(),
    ...
    // Zero Inline SQL: Usar sp_Visita_ObtenerListado
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Visita_ObtenerListado_rust @top=100", &[]).await;
    Json(serde_json::json!({"success": true, "items": items})).into_response()
}
...
pub async fn visita_admin_reportes_km(
    ...
    let mut client = match state.pool.get().await {
        Ok(c) => c,
        Err(e) => return (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(crate::models::ApiResponse::error(e.to_string(), 500))).into_response(),
    };
    ...
    while fecha_cursor <= end {
        ...
        for usuario in &usuarios {
            ...
            if let Some(stats) = diario.first() {
                let km_total = stats.get("km_total").and_then(|v| v.as_f64()).unwrap_or(0.0);
                let puntos_totales = stats.get("puntos_totales").and_then(|v| v.as_i64()).unwrap_or(0);
    ...
    (axum::http::StatusCode::OK, Json(crate::models::ApiResponse::success(serde_json::json!(reporte)))).into_response()
}
...
pub async fn visita_admin_importar_clientes(
    ...
    let clientes_json = body.get("clientes").unwrap_or(&serde_json::json!([])).to_string();
    
    match client.execute("EXEC sp_vc_importar_clientes_rust @P1", &[&clientes_json]).await {
...
pub async fn visita_admin_crear_cliente(
    ...
    match client.query("EXEC sp_vc_cliente_crear_rust @P1, @P2, @P3, @P4, @P5, @P6, @P7, @P8, @P9",
        &[&dto.codigo, &dto.nombre, &dto.direccion, &dto.telefono, &dto.contacto, &lat, &long, &radio_metros, &dto.zona]).await {
        Ok(st) => {
...
pub async fn visita_admin_eliminar_cliente(
    ...
    match client.execute("EXEC sp_vc_cliente_eliminar_rust @P1", &[&id]).await {
        Ok(_) => (axum::http::StatusCode::OK, Json(serde_json::json!({"ok": true}))).into_response(),
        Err(e) => (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
    }
}
...
pub async fn visita_admin_crear_agenda(
    ...
    let fecha_dt = match chrono::NaiveDateTime::parse_from_str(&format!("{} 00:00:00", dto.fecha), "%Y-%m-%d %H:%M:%S") {
        Ok(f) => f,
        Err(_) => return (axum::http::StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "Invalid date"}))).into_response(),
    };

    match client.query("EXEC sp_vc_agenda_crear_rust @P1, @P2, @P3, @P4, @P5", &[&dto.carnet, &dto.cliente_id, &fecha_dt, &dto.orden, &dto.notas]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    return (axum::http::StatusCode::OK, Json(crate::handlers::equipo::row_to_json(&r))).into_response();
...
pub async fn visita_admin_reordenar_agenda(
    ...
    let nuevo_orden = body.get("nuevo_orden").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
    let res = match client.execute("EXEC sp_vc_agenda_reordenar_rust @P1, @P2", &[&id, &nuevo_orden]).await {
        Ok(_) => (axum::http::StatusCode::OK, Json(serde_json::json!({"ok": true}))).into_response(),
        Err(e) => (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
    };
    res
...
pub async fn visita_admin_eliminar_agenda(
    ...
    let res = match client.execute("EXEC sp_vc_agenda_eliminar_rust @P1", &[&id]).await {
        Ok(_) => (axum::http::StatusCode::OK, Json(serde_json::json!({"ok": true}))).into_response(),
        Err(e) => (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
    };
    res
...
pub async fn visita_admin_listar_metas(
    ...
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Visita_ObtenerMetas_rust @carnet=@P1", &[&carnet]).await;
...
pub async fn visita_admin_set_meta(
    ...
    let vd = dto.vigente_desde.and_then(|d| chrono::NaiveDate::parse_from_str(&d, "%Y-%m-%d").ok().map(|dt| dt.and_hms_opt(0,0,0).unwrap()));
    let vh = dto.vigente_hasta.and_then(|d| chrono::NaiveDate::parse_from_str(&d, "%Y-%m-%d").ok().map(|dt| dt.and_hms_opt(0,0,0).unwrap()));

    match client.query("EXEC sp_vc_meta_set_rust @P1, @P2, @P3, @P4, @P5", &[&dto.carnet, &dto.meta_visitas, &costo_km, &vd, &vh]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    return (axum::http::StatusCode::OK, Json(crate::handlers::equipo::row_to_json(&r))).into_response();
