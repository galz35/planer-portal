    ...
    let mut client = match state.pool.get().await {
        ...
        Err(e) => {
            return (
                ...
                Json(crate::models::ApiResponse::error(format!("Error de conexiÃ³n: {}", e), 500)),
            ).into_response()
        }
...
pub async fn marcaje_summary(
    ...
    let recordsets = crate::handlers::equipo::exec_sp_multi_to_json(&mut client, "EXEC sp_marcaje_resumen_diario_rust @P1", &[&carnet]).await;
    
    let history = recordsets.get(0).cloned().unwrap_or_default();
    let flags = recordsets.get(1).and_then(|rs| rs.first()).cloned().unwrap_or(serde_json::json!({
        "isClockedIn": false, 
        "staleShift": false
    }));
    let last_checkin = flags.get("lastCheckIn").cloned().unwrap_or(serde_json::json!(null));
    let last_checkout = flags.get("lastCheckOut").cloned().unwrap_or(serde_json::json!(null));
...
pub async fn marcaje_undo_last(
    ...
    let mut client = match state.pool.get().await {
        Ok(c) => c,
        Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"ok": false, "mensaje": e.to_string()}))).into_response(),
    };

    let carnet = user.carnet().to_string();

    match client.query("EXEC sp_marcaje_deshacer_ultimo_rust @P1", &[&carnet]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    let ok = r.try_get::<bool, _>("ok").ok().flatten().unwrap_or(false);
                    let mensaje = r.try_get::<&str, _>("mensaje").ok().flatten().unwrap_or("Sin resultado").to_string();
                    let tipo_eliminado = r.try_get::<&str, _>("tipo_eliminado").ok().flatten().or_else(|| r.try_get::<&str, _>("tipo_actual").ok().flatten()).unwrap_or("").to_string();
    ...
    (StatusCode::OK, Json(serde_json::json!({"ok": false, "mensaje": "Sin resultado"}))).into_response()
}
...
pub async fn marcaje_request_correction(
    ...
    let carnet = user.carnet().to_string();
    let tipo_solicitud = body.tipo_solicitud.as_deref().unwrap_or("CORRECCION_ASISTENCIA").to_string();
    let motivo = body.motivo.clone();
    let asistencia_id = body.asistencia_id;

    match client.query("EXEC sp_marcaje_solicitar_correccion_rust @P1, @P2, @P3, @P4", &[&carnet, &asistencia_id, &tipo_solicitud, &motivo]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
...
pub async fn marcaje_gps_track(
    ...
    match client.execute("EXEC sp_marcaje_gps_batch_rust @P1, @P2", &[&carnet, &pts_str]).await {
...
pub async fn marcaje_gps_track_batch(
    ...
    match client.query("EXEC sp_marcaje_gps_batch_rust @P1, @P2", &[&carnet, &pts_str]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    let insertados = r.try_get::<i32, _>("insertados").ok().flatten().unwrap_or(0);
                    return (StatusCode::OK, Json(serde_json::json!({"insertados": insertados}))).into_response();
                }
            }
        }
        Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"ok": false, "error": e.to_string()}))).into_response(),
    ...
    (StatusCode::OK, Json(serde_json::json!({"insertados": body.puntos.len()}))).into_response()
}
...
pub async fn marcaje_admin_solicitudes(
    ...
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_ObtenerSolicitudes_rust", &[]).await;
...
pub async fn marcaje_admin_sites(
    ...
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_ObtenerSites_rust", &[]).await;
    Json(crate::models::ApiResponse::success(items)).into_response()
}
...
pub async fn marcaje_admin_ips(
    ...
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_ObtenerIps_rust", &[]).await;
    Json(crate::models::ApiResponse::success(items)).into_response()
}
...
pub async fn marcaje_admin_devices(
    ...
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_ObtenerDevices_rust", &[]).await;
    Json(crate::models::ApiResponse::success(items)).into_response()
}
...
pub async fn marcaje_admin_config(
    ...
    let items = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_ObtenerConfigResumen_rust", &[]).await;
    Json(crate::models::ApiResponse::success(items)).into_response()
}
...
pub async fn marcaje_admin_monitor(
    ...
    if let Some(ref f) = fecha_str {
        if let Ok(st) = client.query("EXEC sp_marcaje_monitor_dia_rust @P1", &[f]).await {
            if let Ok(rows) = st.into_first_result().await {
                items = rows.into_iter().map(|r| crate::handlers::equipo::row_to_json(&r)).collect();
            }
        }
    } else {
        if let Ok(st) = client.query("EXEC sp_marcaje_monitor_dia_rust NULL", &[]).await {
            if let Ok(rows) = st.into_first_result().await {
                items = rows.into_iter().map(|r| crate::handlers::equipo::row_to_json(&r)).collect();
            }
        }
...
pub async fn marcaje_admin_dashboard(
    ...
    if let Some(ref f) = fecha_str {
        if let Ok(st) = client.query("EXEC sp_marcaje_dashboard_kpis_rust @P1", &[f]).await {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
        ...
        if let Ok(st) = client.query("EXEC sp_marcaje_dashboard_kpis_rust NULL", &[]).await {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    result = crate::handlers::equipo::row_to_json(&r);
                }
...
pub async fn marcaje_admin_resolver_solicitud(
    ...
    let accion = if body.aprobado { "APROBADA" } else { "RECHAZADA" };
    match client.query("EXEC sp_marcaje_resolver_solicitud_rust @P1, @P2, @P3, @P4", &[&id, &accion, &body.comentario, &admin_carnet]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    let json = crate::handlers::equipo::row_to_json(&r);
...
pub async fn marcaje_admin_delete_asistencia(
    ...
    match client.query("EXEC sp_marcaje_admin_eliminar_rust @P1, @P2, @P3", &[&id, &admin_carnet, &motivo]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    let json = crate::handlers::equipo::row_to_json(&r);
...
pub async fn marcaje_admin_reiniciar(
    ...
    let admin_carnet = user.carnet().to_string();
    let motivo = body_opt.and_then(|Json(b)| b.motivo);
    match client.query("EXEC sp_marcaje_admin_reiniciar_rust @P1, @P2, @P3", &[&carnet, &admin_carnet, &motivo]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    let json = crate::handlers::equipo::row_to_json(&r);
...
pub async fn marcaje_admin_reportes(
    ...
    if let (Some(fi), Some(ff), Some(car)) = (&fecha_inicio, &fecha_fin, &carnet) {
        if let Ok(st) = client.query("EXEC sp_marcaje_reporte_asistencia_rust @P1, @P2, @P3", &[fi, ff, car]).await {
            if let Ok(rows) = st.into_first_result().await {
                items = rows.into_iter().map(|r| crate::handlers::equipo::row_to_json(&r)).collect();
            }
        }
    } else if let (Some(fi), Some(ff)) = (&fecha_inicio, &fecha_fin) {
        if let Ok(st) = client.query("EXEC sp_marcaje_reporte_asistencia_rust @P1, @P2, NULL", &[fi, ff]).await {
            if let Ok(rows) = st.into_first_result().await {
                items = rows.into_iter().map(|r| crate::handlers::equipo::row_to_json(&r)).collect();
            }
        }
    } else {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "fechas obligatorias"}))).into_response();
    }
    Json(crate::models::ApiResponse::success(items)).into_response()
}
...
pub async fn marcaje_admin_create_site(
    ...
    match client.query("EXEC sp_marcaje_admin_crud_site_rust 'CREAR', NULL, @P1, @P2, @P3, @P4, @P5, NULL", &[&body.nombre, &body.lat, &body.lng, &radio, &acc]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
...
pub async fn marcaje_admin_update_site(
    ...
    let activo_i32 = if body.activo.unwrap_or(true) { 1_i32 } else { 0_i32 };
    match client.query("EXEC sp_marcaje_admin_crud_site_rust 'EDITAR', @P1, @P2, @P3, @P4, @P5, @P6, @P7", &[&id, &body.nombre, &body.lat, &body.lng, &body.radio_metros, &body.accuracy_max, &activo_i32]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    return (StatusCode::OK, Json(crate::handlers::equipo::row_to_json(&r))).into_response();
...
pub async fn marcaje_admin_create_ip(
    ...
    let _ = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_GestionIp_rust @accion='CREAR', @nombre=@P1, @ip=@P2", &[&body.nombre, &body.ip]).await;
...
pub async fn marcaje_admin_delete_ip(
    ...
    // Zero Inline SQL: Usar sp_Marcaje_Admin_GestionIp
    let _ = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_GestionIp_rust @accion='ELIMINAR', @id=@P1", &[&id]).await;
    (StatusCode::OK, Json(serde_json::json!({"ok": true}))).into_response()
}
...
pub async fn marcaje_admin_update_device(
    ...
    match client.query("EXEC sp_marcaje_admin_device_rust @P1, @P2", &[&uuid, &estado]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    return (StatusCode::OK, Json(crate::handlers::equipo::row_to_json(&r))).into_response();
...
pub async fn marcaje_geocerca_validar(
    ...
    let carnet = user.carnet().to_string();
    match client.query("EXEC sp_marcaje_validar_geocerca_rust @P1, @P2, @P3", &[&carnet, &body.lat, &body.lng]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    return (StatusCode::OK, Json(crate::handlers::equipo::row_to_json(&r))).into_response();
    ...
    (StatusCode::OK, Json(serde_json::json!({"dentro_geocerca": true, "estado": "SIN_RESTRICCION"}))).into_response()
}
...
pub async fn marcaje_admin_geocercas(
    ...
    let mut items = Vec::new();
    if let Ok(st) = client.query("EXEC sp_marcaje_geocercas_usuario_rust @P1", &[&carnet]).await {
        if let Ok(rows) = st.into_first_result().await {
            items = rows.into_iter().map(|r| crate::handlers::equipo::row_to_json(&r)).collect();
        }
    }
...
pub async fn marcaje_admin_create_geocerca(
    ...
    let _ = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_GestionGeocerca_rust @accion='CREAR', @carnet=@P1, @id_site=@P2", &[&body.carnet, &body.id_site]).await;
    (StatusCode::OK, Json(serde_json::json!({"ok": true}))).into_response()
}
...
pub async fn marcaje_admin_delete_geocerca(
    ...
    // Zero Inline SQL: Usar sp_Marcaje_Admin_GestionGeocerca
    let _ = crate::handlers::equipo::exec_sp_to_json(&mut client, "EXEC sp_Marcaje_Admin_GestionGeocerca_rust @accion='BORRAR', @id=@P1", &[&id]).await;
    (StatusCode::OK, Json(serde_json::json!({"ok": true}))).into_response()
}
