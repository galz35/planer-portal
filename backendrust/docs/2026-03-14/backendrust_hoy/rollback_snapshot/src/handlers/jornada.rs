...
pub async fn jornada_patrones(
    ...
    // simplified fetch, nested details is harder directly in one query
    let response = match client
        .query(
        ...
        .await
    {
        Ok(stream) => match stream.into_first_result().await {
            Ok(rows) => {
                ...
                for r in rows {
                    items.push(serde_json::json!({
                        "id_patron": r.get::<i32, _>("id_patron"),
                        "nombre": r.get::<&str, _>("nombre"),
...
pub async fn jornada_crear_horario(
    ...
    let nocturno = if dto.es_nocturno.unwrap_or(false) { 1_i32 } else { 0_i32 };
    ...
    match client.query(query, &[&dto.nombre, &dto.hora_entrada, &dto.hora_salida, &duracion, &nocturno, &tolerancia, &descanso]).await {
        Ok(st) => {
...
pub async fn jornada_actualizar_horario(
    ...
    match client.execute(query, &[&dto.nombre, &dto.hora_entrada, &dto.hora_salida, &dur_f32, &noc_i32, &dto.tolerancia_min, &dto.descanso_min, &id]).await {
...
pub async fn jornada_crear_patron(
    ...
    match client.query(query_patron, &[&dto.nombre, &dto.total_dias, &dto.descripcion]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
    ...
    if id_patron == 0 {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "Failed to create patron"}))).into_response();
    }
    ...
    (StatusCode::OK, Json(serde_json::json!({"ok": true, "id_patron": id_patron}))).into_response()
}
...
pub async fn jornada_crear_asignacion(
    ...
    let fecha_inicio = match chrono::NaiveDate::parse_from_str(&dto.fecha_inicio, "%Y-%m-%d") {
        Ok(f) => f.and_hms_opt(0,0,0).unwrap(),
        Err(_) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "Invalid date"}))).into_response(),
    };
    
    let fecha_fin = dto.fecha_fin.and_then(|f| chrono::NaiveDate::parse_from_str(&f, "%Y-%m-%d").ok().map(|d| d.and_hms_opt(0,0,0).unwrap()));
    ...
    match client.query(query, &[&dto.carnet, &dto.id_patron, &fecha_inicio, &fecha_fin]).await {
        Ok(st) => {
            if let Ok(rows) = st.into_first_result().await {
                if let Some(r) = rows.into_iter().next() {
                    return (StatusCode::OK, Json(crate::handlers::equipo::row_to_json(&r))).into_response();
