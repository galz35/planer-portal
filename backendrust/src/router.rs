...
pub fn api_router(
    ...
    Router::new()
        ...
        .route("/auth/sso-login", post(auth_sso_login))
        .route("/auth/portal-session", post(auth_portal_session))
        .route("/auth/sso-sync-user", post(auth_sso_sync_user))
        ...
        .route(
            "/tareas/:id",
            get(tareas_get).patch(tareas_update).delete(tareas_delete),
        )
        ...
        .route(
            "/marcaje/request-correction",
            post(marcaje_request_correction),
        )
        ...
        .route(
            "/visita-campo/tracking-batch",
            post(visita_campo_tracking_batch),
        )
        .route("/visita-campo/stats/km", get(visita_campo_stats_km))
        .route("/visita-campo/tracking-raw", get(visita_campo_tracking_raw))
        .route(
            "/visita-campo/usuarios-tracking",
            get(visita_campo_usuarios_tracking),
        )
        // --- Jornada ---
        ...
        .route(
            "/visita-admin/importar-clientes",
            post(visita_admin_importar_clientes),
        )
        ...
        .route(
            "/visita-admin/tracking/:carnet",
            get(visita_admin_tracking_usuario),
        )
        .route("/visita-admin/agenda", post(visita_admin_crear_agenda))
        .route(
            "/visita-admin/agenda/:id_or_carnet",
            get(visita_admin_listar_agenda).delete(visita_admin_eliminar_agenda),
        )
        .route(
            "/visita-admin/agenda/:id/reordenar",
            put(visita_admin_reordenar_agenda),
        )
        .route(
            "/visita-admin/metas",
            get(visita_admin_listar_metas).post(visita_admin_set_meta),
        )
        ...
        .route("/tasks/:id/clone", post(planning_clone_task))
        ...
        .route(
            "/acceso/organizacion/buscar",
            get(acceso_organizacion_buscar),
        )
        .route(
        ...
        .route("/foco", get(foco_list).post(foco_create))
        .route("/foco/:id", patch(foco_update).delete(foco_delete))
        ...
        .route("/mi-dia/checkin", post(mi_dia_checkin))
        ...
        .route(
            ...
            post(notificacion_registrar_token),
        )
        ...
        .route(
            ...
            get(reportes_equipo_performance),
        )
...
fn admin_subrouter(_state: ApiState) -> Router<ApiState> {
    Router::new()
        ...
        .route(
            "/usuarios-organizacion/:idUsuario/:idNodo",
            delete(admin_delete_usuario_organizacion),
        )
        ...
        .route(
            "/import/template/empleados",
            get(admin_import_template_empleados),
        )
        .route(
            "/import/template/organizacion",
            get(admin_import_template_organizacion),
        )
        ...
        .route(
            "/security/assign-menu/:id",
            delete(admin_security_delete_assign_menu),
        )
