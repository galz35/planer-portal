...
pub async fn equipo_hoy(
    ...
    // Mood summary
    let animos: Vec<&str> = checkins
        .iter()
        .filter_map(|c| c.get("estadoAnimo").and_then(|v| v.as_str()))
        .collect();
    let feliz = animos
        .iter()
        .filter(|a| **a == "Tope" || **a == "Bien")
        .count();
    let neutral = animos
        .iter()
        .filter(|a| **a == "Neutral" || a.is_empty())
        .count();
    let triste = animos.iter().filter(|a| **a == "Bajo").count();
    let promedio = if !result_miembros.is_empty() {
        (animos.len() as f64 / result_miembros.len() as f64) * 100.0
    } else {
        0.0
    };
...
pub async fn equipo_inform(
    ...
    let miembros = exec_sp_to_json(
        &mut client,
        "EXEC sp_Usuarios_ObtenerDetallesPorCarnets_rust @P1",
        &[&carnets_csv.as_str()],
    )
    .await;
    let checkins = exec_sp_to_json(
        &mut client,
        "EXEC sp_Checkins_ObtenerPorEquipoFecha_rust @P1, @P2",
        &[&carnets_csv, &fecha],
    )
    .await;
    let stats = exec_sp_to_json(
        &mut client,
        "EXEC sp_Equipo_ObtenerInforme_rust @P1, @P2",
        &[&carnets_csv, &fecha],
    )
    .await;
...
/// GET /equipo/backlog
pub async fn equipo_backlog(user: AuthUser, State(state): State<ApiState>) -> impl IntoResponse {
    let carnet = user.carnet().to_string();
    let mut client = match state.pool.get().await {
        Ok(c) => c,
        Err(e) => {
...
pub async fn equipo_miembro(
    ...
    if member_carnet.is_empty() {
        return (
            ...
            Json(serde_json::json!({"message": "Miembro no encontrado"})),
        )
            .into_response();
    }

    let details = exec_sp_to_json(
        &mut client,
        "EXEC sp_Usuarios_ObtenerDetallesPorCarnets_rust @P1",
        &[&member_carnet.as_str()],
    )
    .await;
    if let Some(mut emp) = details.into_iter().next() {
        ...
        (
            StatusCode::OK,
            Json(crate::models::ApiResponse::success(emp)),
        )
            .into_response()
    } else {
        (
            ...
            Json(crate::models::ApiResponse::error(
                "Detalles de miembro no encontrados".to_string(),
                404,
            )),
        )
            .into_response()
...
pub async fn equipo_miembro_tareas(
    ...
    let member_carnet = get_carnet_by_id(&mut client, id_usuario).await;
    if member_carnet.is_empty() {
        return Json(crate::models::ApiResponse::success(serde_json::json!([]))).into_response();
    }

    let tareas = exec_sp_to_json(
        &mut client,
        "EXEC sp_Tareas_ObtenerPorUsuario_rust @P1, NULL, NULL, NULL, NULL, NULL",
        &[&member_carnet.as_str()],
    )
    .await;
    Json(crate::models::ApiResponse::success(tareas)).into_response()
}
...
pub async fn equipo_actividad(
    ...
    let query = format!(
        ...
        if !search.is_empty() {
            format!(
                "AND (al.accion LIKE '%{}%' OR al.detalle LIKE '%{}%')",
                search, search
            )
        } else {
            String::new()
    ...
    Json(crate::models::ApiResponse::success(
        serde_json::json!({ "items": rows, "total": rows.len(), "page": page, "limit": limit }),
    ))
    .into_response()
}
...
pub async fn equipo_actividad_detail(
    ...
    let rows = exec_query_to_json(
        &mut client,
        "SELECT * FROM p_AuditLogs WHERE idLog = @P1",
        &[&id_str],
    )
    .await;
...
pub async fn get_visible_carnets(
    ...
    let stream = client
        ...
        .await
        .map_err(|e: tiberius::error::Error| e.to_string())?;
    let rows: Vec<Row> = stream
        .into_first_result()
        .await
        .map_err(|e: tiberius::error::Error| e.to_string())?;
    let mut carnets: Vec<String> = rows
        .iter()
        .filter_map(|r: &Row| {
            r.try_get::<&str, _>("carnet")
                .ok()
                .flatten()
...
pub async fn get_carnet_by_id(client: &mut PoolConn<'_>, id_usuario: i32) -> String {
    match client
        .query(
...
/// Execute SP and return results as Vec<serde_json::Value>, surfacing DB errors to the caller.
pub async fn exec_sp_to_json_result(
    client: &mut PoolConn<'_>,
    sp_call: &str,
    params: &[&dyn tiberius::ToSql],
) -> Result<Vec<serde_json::Value>, String> {
    let stream = client
        .query(sp_call, params)
        .await
        .map_err(|e| format!("Error ejecutando SP {}: {}", sp_call, e))?;
    let rows: Vec<Row> = stream
        .into_first_result()
        .await
        .map_err(|e| format!("Error procesando resultado de SP {}: {}", sp_call, e))?;
    Ok(rows.iter().map(|r: &Row| row_to_json(r)).collect())
}
...
/// Execute SP and return multiple recordsets, surfacing DB errors to the caller.
pub async fn exec_sp_multi_to_json_result(
    client: &mut PoolConn<'_>,
    sp_call: &str,
    params: &[&dyn tiberius::ToSql],
    ...
    while let Some(item) = stream.next().await {
        match item {
            ...
            Err(e) => {
                return Err(format!(
                    "Error procesando recordsets de SP {}: {}",
                    sp_call, e
                ));
            }
        }
    ...
    for (i, col) in row.columns().iter().enumerate() {
        ...
        let val: serde_json::Value = match col.column_type() {
            ...
            tiberius::ColumnType::Decimaln | tiberius::ColumnType::Numericn => {
                if let Ok(Some(v)) = row.try_get::<tiberius::numeric::BigDecimal, _>(i) {
            ...
            tiberius::ColumnType::Guid => row
                .try_get::<tiberius::Uuid, _>(i)
                .ok()
                .flatten()
                .map(|v| serde_json::Value::String(v.to_string()))
                .unwrap_or(serde_json::Value::Null),
            tiberius::ColumnType::DatetimeOffsetn => row
            ...
            _ => {
                if let Ok(Some(dt)) = row.try_get::<chrono::NaiveDateTime, _>(i) {
                    serde_json::Value::String(format!("{}Z", dt.format("%Y-%m-%dT%H:%M:%S")))
                } else if let Ok(Some(d)) = row.try_get::<chrono::NaiveDate, _>(i) {
                    serde_json::Value::String(d.format("%Y-%m-%d").to_string())
                } else if let Ok(Some(t)) = row.try_get::<chrono::NaiveTime, _>(i) {
